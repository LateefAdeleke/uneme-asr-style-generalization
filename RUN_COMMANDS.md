...
## 8) Additional commands for E4 and E5

### E4 smoke
```bash
PYTHONPATH=src python scripts/run_whisper_pipeline.py --experiments E4 --smoke-test --skip-audio-check --smoke-max-rows 8
```

### E5 smoke
```bash
PYTHONPATH=src python scripts/run_whisper_pipeline.py --experiments E5 --smoke-test --skip-audio-check --smoke-max-rows 8
```
...
Expected output dirs from registry:
